...
namespace OOPCS.day4.Polymorphism
{
    public class BankBranch
    {
        ...
        public double TotalDeposits()
        {
            ...
            foreach(Account account in _accounts)
            {
                if (account.Balance > 0) totalDeposits += account.Balance;
            }
        ...
        public double TotalInterestPaid()
        {
            ...
            foreach (Account account in _accounts)
            {
                if( account.Balance > 0 ) totalInterestPaid += account.CalculateInterest();
            }
        ...
        public double TotalInterestEarned()
        {
            ...
            foreach (Account account in _accounts)
            {
                if (account.Balance < 0) totalInterestEarned += account.CalculateInterest();
            }
        ...
        public void PrintCustomers()
        {
            ...
            foreach(Account account in _accounts)
            {
                if(uniqueIds.Exists(x => x == account.AcctHolderId) == false)
                {
                    Console.Write(account.AcctHolderId + " , ");
                    uniqueIds.Add(account.AcctHolderId);
                }
            }
            Console.WriteLine();
        }
